...
function Registry.setObject(object, name)
    assert(
        ...
        string.format('Registry.setObject() - object "%s" already exists', tostring(name))
    )
...
function Registry.getObject(name)
    assert(
        Registry.objects_[name] ~= nil,
        string.format('Registry.getObject() - object "%s" not exists', tostring(name))
    )
...
function Registry.removeObject(name)
    assert(
        Registry.objects_[name] ~= nil,
        string.format('Registry.removeObject() - object "%s" not exists', tostring(name))
    )
    Registry.objects_[name] = nil
end
