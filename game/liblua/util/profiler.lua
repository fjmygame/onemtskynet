...
function profiler:stop(mode)
    ...
    for _, report in ipairs(self._REPORTS) do
        ...
        -- trace
        local str = string.safeFormat("%6.3f, %6.2f%%, %7d, %s", report.totaltime, percent, report.callcount, report.title)
        log.Debug("gg", "[", str, "]")
...
function profiler:_func_title(funcinfo)
    ...
    -- the function line
    local line = string.safeFormat("%d", funcinfo.linedefined or 0)
    ...
    -- make title
    return string.safeFormat("%-30s: %s: %s", name, source, line)
end
