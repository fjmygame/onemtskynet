-- --------------------------------------
-- Create Date:2022-11-29 15:20:35
-- Author  : sgys
-- Version : 1.0
-- Filename: LRUCacheExt.lua
-- Introduce  : 类介绍
-- --------------------------------------
...
---@class LRUCacheExt
local LRUCache = BuildOther("LRUCacheExt")

function LRUCache:getNowTime()
    return skynet.now() / 100
end

function LRUCache:New(capacity, delayTime)
    ...
    object.count = 0
    object.delayTime = delayTime or 86400 -- 默认延时一天卸载

    -- 定时检查
    skynet.timeout(30000, handlerName(object, "timeoutCheck"))
    return object
end

-- 超时检查
function LRUCache:timeoutCheck()
    local time = self:getNowTime()
    local rmKeys = {}
    local key2Node = self.key2Node
    for key, node in pairs(key2Node) do
        if time - node.time > self.delayTime then
            rmKeys[#rmKeys + 1] = key
        end
    end
    if next(rmKeys) then
    ...
    skynet.timeout(30000, handlerName(self, "timeoutCheck"))
end
...
-- 设置是否更新超时时间
function LRUCache:SetIsUpdateTimeOut(isUpdate)
    self.isUpdateTimeOut = isUpdate
end
    ...
    if not self.cacheHead then
        ...
        if not node then
            if self.count == self.maxSize and self.cacheHead and self.cacheHead == self.cacheTail then
                ...
                self.cacheHead.time = self:getNowTime()
                self.key2Node[key] = self.cacheHead
            else
                while self.count >= self.maxSize do
                    local rmNode = self.cacheTail
                    self.key2Node[rmNode.key] = nil
                    rmNode.preNode.nextNode = nil
                    self.cacheTail = rmNode.preNode
                    self.count = self.count - 1

                    table.insert(callbackCache, rmNode)
                end
                local node1 = {
                    key = key,
                    value = value,
                ...
                self.cacheHead.preNode = node1
                self.cacheHead = node1
                self.key2Node[key] = node1
                self.count = self.count + 1
            end
        else
            node.value = value
...
-- 清空数据
function LRUCache:clean()
    if self.removeFunc then -- 处理回调【可能需要落地】
        for _, node in pairs(self.key2Node) do
            serviceFunctions.fork(self.removeFunc, node.key, node.value)
        end
    end
    self.key2Node = {}
    self.cacheHead = nil
