...
// tskiplist similar with the version in redis
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
tskiplistNode *slCreateNode(int level, double score, tslobj *obj)
{
    tskiplistNode *n = malloc(sizeof(*n) + level * sizeof(struct skiplistLevel));
    n->score = score;
    n->obj = obj;
    return n;
}
...
void slFreeNode(tskiplistNode *node)
{
    slFreeObj(node->obj);
    free(node);
}
...
{
    tskiplistNode *node = sl->header->level[0].forward, *next;
...
int compareslObj(tslobj *a, tslobj *b)
{
...
int equalslObj(tslobj *a, tslobj *b)
{
...
{
    tskiplistNode *update[SKIPLIST_MAXLEVEL], *x;
...
void slDeleteNode(tskiplist *sl, tskiplistNode *x, tskiplistNode **update)
{
    int i;
...
/* Delete an element with matching score/object from the tskiplist. */
...
/* Delete all the elements with rank between start and end from the tskiplist.
...
unsigned long slDeleteByRank(tskiplist *sl, unsigned int start, unsigned int end, slDeleteCb cb, void *ud)
{
    ...
    {
        tskiplistNode *next = x->level[0].forward;
        slDeleteNode(sl, x, update);
        cb(ud, x->obj);
        slFreeNode(x);
        removed++;
...
int slIsInRange(tskiplist *sl, double min, double max)
{
    tskiplistNode *x;
...
void slDump(tskiplist *sl)
{
    ...
    while (x->level[0].forward)
    {
        x = x->level[0].forward;
        i++;
        printf("node %d: score:%f, member:%s, timestamp:%f\n", i, x->score, x->obj->ptr, x->obj->timestamp);
    }
}
